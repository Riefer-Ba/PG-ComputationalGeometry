...
public class ClusterMerge {
		...
		public ArrayList<Punkt> traverse = new ArrayList<Punkt>();
		public double size;
...
//			//verbindet Cluster, behaltet Tsp Eigenschaften bei.
			...
			System.out.println("Tourl‰nge: "+FinalTsp.get(0).TourLaenge(FinalTsp));
		}
		...
		//	Arrays.fill(combined, 0);
			...
			//2.mst vom jeweiligen bestedge eckpkt lˆsen
			combined[a][c] = 0;
			combined[c][a] = 0;
		...
		private LinienSegment[] candidate(Punkt[] p1, Punkt[] p2, LinienSegment mst) {
			...
			boolean c0 =false ,c1 = false;
			 ...
			 if( kreuzungsfreiBool(c12, mst)  == true && kreuzungsfreiBool(c12, m21) == true && kreuzungsfreiBool(c12, m22) == true) {
				 ...
				 else { //wenn beide g¸ltig dann nimm die k¸rzere
...
//			 }
		...
		public void printTour(ArrayList<double[][]> tsp, List<List<Punkt>> clusters) {
			...
			System.out.println("finale Tourl‰nge: "+FinalTsp.get(0).TourLaenge(FinalTsp));
			size = FinalTsp.get(0).TourLaenge(FinalTsp);
		}
		...
		private boolean ValidTourCheck(int[][] indexTour, List<Punkt> cluster) {
			...
			traverse.add(cluster.get(indexTour[0][1]));
				...
				for(int j=0; j< m; j++) {
					if( visited[j] == false) {
						...
						if( indexTour[j][0] == current) {
							...
							traverse.add(cluster.get(indexTour[j][1]));
							found = true;
							break;
						}
							...
							traverse.add(cluster.get(indexTour[j][0]));
							found = true;
							break;
						}
